...
//             reject("Problem already exists"); // 问题已经存在就reject
//         } else {
//             newProblem.id = problems.length + 1; // 为新问题分配id
...
var addProblem = function(newProblem) {
    return new Promise(function(resolve, reject) {
        ProblemModel.findOne({name: newProblem.name}, function (err, problem) { // 基于name去找是否已存在
            if (problem) {
                reject("Problem already exists"); // 问题已经存在就reject
            } else {
                ProblemModel.count({}, function (err, num) {
                    newProblem.id = num + 1; // 为新问题分配id
                    var newMongoProblem = new ProblemModel(newProblem); // 将JS的newProblem转换成MongoDB的newProblem
                    newMongoProblem.save(); // 保存
                    resolve(newProblem);
                });
            }
        })
